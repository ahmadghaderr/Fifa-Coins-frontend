...
const LoginSignup = () => {
  ...
  const phoneRef = useRef(null); // 🆕 Added
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isLogin) {
        ...
        const token = signupResponse.data.accessToken || signupResponse.data.token || signupResponse.data.access_token;
        const decoded = jwtDecode(token);
        const roleFromToken = decoded.role;
        const isAdmin = roleFromToken === 'admin';
        localStorage.setItem('token', token);
            ...
            <>
              <div className="input-field">
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && phoneRef.current?.focus()} // 🆕 updated
                />
              </div>
